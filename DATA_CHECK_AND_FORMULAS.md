...
- `LPP Received` = money actually collected against accrued LPP (manual entry) with its `Received Date`
- `Outstanding = Total Receivables + LPP − Total Received`
...
- LPP = `ROUND(Mat.Value × weeks × 0.0118, 0)` — delay counted from **22 Aug 2026**; waived if FP received on/before **24 Aug 2026**
